...
> = {
  ...
  /** Values to filter out from the options */
  filterValues?: string[];
};
...
>({
  ...
  optionLabel,
  filterValues,
}: MultipleSelectorProps<TData, TOptionValue, TOptionLabel>) {
    ...
    () =>
      query.data?.pages
        ...
        .map((item) => ({
          ...
          disable: filterValues?.includes(String(item[optionValue])),
        })) || [],
    [query.data, optionValue, optionLabel, filterValues]
  );
  ...
  const EmptyItem = React.useCallback(() => {
    ...
    // Show loading state or empty indicator in CommandEmpty
    return (
      <CommandEmpty>
        {query.isPending ? (
  ...
  }, [creatable, emptyIndicator, onSearch, options, query.isPending]);
  ...
  return (
    ...
    >
      <div
        className={cn(
          'border-input bg-input/30 ring-offset-background focus-within:ring-ring grid min-h-9 items-center rounded-md border text-base focus-within:ring-2 md:text-sm',
          {
      ...
      >
        <div className='relative flex flex-wrap gap-1'>
          ...
          <button
            ...
            className={cn(
              'hover:bg-muted absolute right-0 grid h-6 w-6 place-items-center rounded p-0',
              (hideClearAllButton ||
                disabled ||
                selected.length < 1 ||
                ...
                {Object.entries(selectables).map(([key, dropdowns]) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {!query.isFetchingNextPage &&
                        !query.hasNextPage &&
                        Object.keys(selectables).length > 0 && (
