...
export default function Page() {
  ...
  const query: Inputs['masterData']['service']['getAll'] = {
    page: pageIndex + 1,
    limit: pageSize,
  ...
  const servicesQuery = useQuery(
    orpc.masterData.service.getAll.queryOptions({
      placeholderData: keepPreviousData,
      input: { ...query, serviceId: Number(serviceId) },
    })
  );
  ...
  const table = useReactTable({
    data: servicesQuery.data?.data ?? defaultData,
    columns,
    pageCount: servicesQuery.data?.pagination?.pageCount ?? -1,
    state: {
      pagination,
    },
  ...
  return (
    <>
      <div className='mb-4 flex flex-wrap items-center justify-between gap-4 pt-4'>
        ...
        <div>
          <Link
            href='/master-data/service/create'
            className={cn(buttonVariants({ variant: 'default' }))}
          >
            <PlusIcon className='size-4' /> <span>New Service</span>
          </Link>
        </div>
      </div>
      ...
      <DataTable tableInstance={table} isPending={servicesQuery.isFetching} />
    </>
  );
}
