...
const searchByOptions = [
  ...
  keyof NonNullable<Inputs['sales']['transaction']['getAll']>
>[];
...
export default function Page() {
  ...
  const query: Inputs['sales']['transaction']['getAll'] = {
    page: pageIndex + 1,
    limit: pageSize,
  ...
  const transactionsQuery = useQuery(
    orpc.sales.transaction.getAll.queryOptions({
  ...
  const table = useReactTable({
    data: transactionsQuery.data?.data ?? defaultData,
    columns,
    pageCount: transactionsQuery.data?.pagination?.pageCount ?? -1,
    state: {
      pagination,
    },
  ...
  return (
    <>
      <div className='mb-4 flex flex-wrap items-center justify-between gap-4 pt-4'>
        ...
        <div>
          <Link
            href='/sales/transaction/create'
            className={cn(buttonVariants({ variant: 'default' }))}
          >
            <PlusIcon className='size-4' /> <span>New Transaction</span>
          </Link>
        </div>
      </div>
      ...
      <DataTable
        tableInstance={table}
        isPending={transactionsQuery.isFetching}
      />
    </>
  );
}
